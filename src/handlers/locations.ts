...
import { users, logtimes } from "@/schema";
...
export const handleLocation = createHandler(updateLogtimeSchema, async (req, res) => {
    ...
    try {
        ...
        let logtime = await db.query.logtimes.findFirst({
            where: (logtimes, {eq, and}) => and(
                eq(logtimes.userId, userId),
                eq(logtimes.date, new Date(date))
            )
        })

        if (logtime) {
            await db.update(logtimes).set({
        ...
        await db.insert(logtimes).values({
            userId, 
            date: new Date(date), 
            time
        }).onConflictDoUpdate({
